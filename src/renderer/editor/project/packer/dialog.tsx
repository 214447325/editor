...
export class PackerDialog extends React.Component<IPackerProps, IPackerState> {
	...
	public constructor(props: IPackerProps) {
		...
		// @ts-ignore
		this._steps = {
			[PackerDialogStep.Building]:
				//@ts-ignore
				<PackerBuilderStep packer={this._packer} onDone={() => this.setState({ currentStep: PackerDialogStep.SelectingFiles })} />,

			[PackerDialogStep.SelectingFiles]:
				//@ts-ignore
				<PackerTreeStep packer={this._packer} onDone={(f) => {
				this.setState({ currentStep: PackerDialogStep.Packing, selectedFiles: f }, () => this.pack());
			}} />,

			[PackerDialogStep.Packing]:
				//@ts-ignore
				<PackerPackStep ref={(r) => this._packerStep = r} packer={this._packer} />,
		}
