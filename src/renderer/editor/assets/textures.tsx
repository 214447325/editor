            ...
            <>
                <div className={Classes.FILL} key="materials-toolbar" style={{ width: "100%", height: "25px", backgroundColor: "#333333", borderRadius: "10px", marginTop: "5px" }}>
                    <ButtonGroup>
                        ...
                        <Divider />
                        <Button key="clear-unused" icon={
                            //@ts-ignore
                            <Icon src="recycle.svg" />} small={true} text="Clear Unused" onClick={() => this._clearUnusedTextures()} />
                    </ButtonGroup>
                </div>
                {super.render()}
            </>
    ...
    public onContextMenu(item: IAssetComponentItem, e: React.MouseEvent<HTMLImageElement, MouseEvent>): void {
        ...
        if (extension === ".env" || extension === ".dds") {
            setAsEnvironmentTexture = (
                <MenuItem text="Set As Environment Texture" icon={
                    //@ts-ignore
                    texture === this.editor.scene!.environmentTexture ? <Icon src="check.svg" /> : undefined} onClick={() => {
        ...
        ContextMenu.show(
            <Menu className={Classes.DARK}>
                ...
                <MenuItem text="Clone..." icon={
                    //@ts-ignore
                    <Icon src="clone.svg" />} onClick={() => this._cloneTexture(texture)} />
                <MenuDivider />
                <MenuItem text="Locked" icon={
                    //@ts-ignore
                    texture.metadata.isLocked ? <Icon src="check.svg" /> : undefined} onClick={() => {
                ...
                <MenuItem text="Remove" icon={
                    //@ts-ignore
                    <Icon src="times.svg" />} onClick={() => this._removeTexture(item, texture)} />
            </Menu>,
